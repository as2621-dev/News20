...
- [Phase 5a](phase-5a-build-your-30-and-entity-ranker.md) — **"Build your 30" allocator + entity-aware ranking** (migration 0008 `user_feed_allocation`): user-set per-category slot budgets + manual sequence drive `daily_feeds`; entity follows fold into the per-story Score as a title/ticker-matched bonus (8 categories as drawn; exactly-one-best-fit bucketing; live `0007`+`0008` apply + e2e). *(Supersedes `phase-5e` + `reference/control-surface-spec.md`.)*
