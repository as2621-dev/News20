...
8. **Personalization = category prioritization from engagement signals** (reuse TLDW `memory/player_signals`), not an ML model. Why: simplicity for v1; surface the categories the user engages with first. Rules out: the 6-signal behavioral ML described in the report.
...
- **M1 — Audio-first karaoke reel MVP:** News ingestion → automated once-per-story pipeline producing the canonical **audio digest + word-timed caption JSON + ambient poster image** in Supabase → Next.js swipe-up reel that **composites karaoke captions live in the client** (sentence dim → current word white → one `#FACC15` keyword/sentence) synced to the TTS audio over a drifting poster wash, running inside a Capacitor iOS build. *True when:* a user can open the app on a phone and passively listen-and-read ~20–30 fresh daily stories back-to-back, captions tracking the audio word-by-word.
...
- **M3 — Voice + personalization + follow + onboarding:** Swipe-left Gemini Live voice mode, category-prioritized feed from engagement signals, follow-a-story + "what's new since you last watched", plus the **voice-agent interest-interview onboarding** (listening orb + hierarchical tappable category taxonomy + niche-down follow-ups, wired to the same Gemini Live stack as Voice mode) and **email-only passwordless magic-link auth** (Supabase email OTP; no Sign-in-with-Apple). *True when:* a new user can onboard by voice into a hierarchical interest profile, the feed adapts to that interest, and a user can interrogate hands-free.
...
- [Phase 1c](phase-1c-capacitor-ios-live-data.md) — Swap fixtures→Supabase + Capacitor iOS build playing seeded digests back-to-back in the Simulator.
- [Phase 1d](phase-1d-daily-content-pipeline.md) — Trigger.dev v4 daily pipeline (ingest→rank→script→TTS→captions→poster→persist) → fresh finite ~20–30-story feed.
- _Suggested order: **Phase 1 ∥ Phase 1b** (parallel; share the `src/types/feed.ts` contract), then **Phase 1c** (needs 1+1b), then **Phase 1d** (needs 1b)._
...
### M3 — Voice + personalization + follow + onboarding
> ⚠ **Depends on M1 (planned: `phase-1`/`1b`/`1c`/`1d`) + M2 (planned, not built).** M3's voice mode reuses M2's RAG endpoint (`phase-2b`); personalization reorders M1's feed; onboarding precedes M1's reel; the M3 schema FKs to M1's `stories`/`segments`. Ship at minimum M1's content migration + reel and M2's RAG endpoint before executing these.
- [Phase 3](phase-3-m3-auth-voice-foundation.md) — Auth (email magic-link) + user-side schema + parameterized Gemini Live transport + shared orb/waveform UI (the rails 3b/3c share).
...
- [Phase 3c](phase-3c-m3-voice-onboarding.md) — Voice-agent interest interview (hierarchical chips + niche-down + function-calling extraction → `user_interest_profile`); typed fallback.
- [Phase 3d](phase-3d-m3-personalization-follow.md) — Interest-weighted feed (world tier + niche tier) from signals/profile + follow-a-story + "what's new since you last watched".
