...
- [~] 4: Profile-update job + per-user allocation → daily_feeds + Trigger.dev fan-out — **PARTIAL (2026-05-31)** — core shipped: `assemble_user_feed` allocator (§3) + idempotent `write_daily_feed` + `assemble_daily_feeds` batch entry + 8 mutation-checked tests; ruff/tsc/biome clean, 129 agents-suite green. **DEFERRED (need budget ack):** profile-update job (`agents/memory/*`), `trigger.config.ts`, `batchTrigger` per-story fan-out, and the live 2-user e2e (the phase-level DoD). `trigger/dailyPipeline.ts` is a documented schedule shell — `@trigger.dev/sdk` not installed, so `trigger/` was excluded from `tsconfig.json`/`biome.json`. Report: `phase-1d-daily-content-pipeline-sub-4.md`. <small>(orig:</small> PENDING (⚠ irreversible: registers the daily cron + first **live GDELT batch at active-interest scale** + paid API + writes). Source is the **same GDELT adapter SP1 built** (`ingest_active_interests(adapter=...)` — GDELT in prod, mock in tests); SP1/SP2/SP3 never made a live GDELT call (SP3's e2e used a static fixture), so SP4 is the first real at-scale ingest. No second source — open item is GDELT *coverage* for niche/financial interests, not source *choice*.
