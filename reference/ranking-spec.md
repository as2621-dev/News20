...
3. **Produce-cap headroom — RETIRED (issue #74, 2026-07-26).** `DEFAULT_HEADROOM_MULTIPLIER` / `PRODUCE_CAP_HEADROOM` are gone and the caps are pure demand. The multiplier existed only to over-provision the **render pool** against post-production attrition; production no longer runs on the capped pool at all — `production_selection.select_stories_for_production` cuts each user's 30 from the pool first, only that union is produced, and a reel that fails is replaced by promoting the next ranked standby in its category. Attrition is repaired, not pre-paid.
