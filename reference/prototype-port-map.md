...
3. **Voice-agent onboarding** (replaces a checkbox grid). See §5 (the prominent voice-onboarding target).
...
| 2 | **Voice-agent onboarding** (interview → live interest profile) | `voiceProfileStep` + `VP_TURNS`/`VP_KW`/`VP_LABEL` (scripted) | `VoiceOnboarding` + shared `VoiceOrb`, `Waveform`, `InterestChips`, `InterestProfilePanel` | `src/components/onboarding/VoiceOnboarding.tsx` | `interests` (read: category→subcategory→sub-sub scaffold for chips); **writes** `user_interest_profile` | `agent_asking` · `your_turn` · `listening` · `responding` · `profile_ready`; voice ↔ typed-fallback. **See §5 — production target supersedes the scripted prototype.** |
...
## 5. Voice-onboarding target (production) — supersedes the scripted prototype

> **This section is the user's final, explicit target for voice onboarding. It SUPERSEDES the prototype's scripted `VP_TURNS` / `voiceProfileStep`.** The prototype hard-codes 3 turns with canned user answers and keyword extraction (`VP_KW`/`extract()`); production replaces that with a real two-way Gemini Live conversation. Reuse the prototype's *visual* template (orb + waveform + lighting-up interest tags) exactly — only the brain changes.
...
**Onboarding flow order** (replacing `onbStep`): splash (1) → **voice onboarding** (2, this section) → email sign-in (3) → loading skeleton → reel.
...
3. **M-Voice+Onboarding — hands-free + first-run.** Shared `VoiceOrb`/`Waveform` template; in-news **Voice mode** (§2 row 7) over Gemini Live (mic-in-orb, listening/responding, RAG-grounded) + the **voice-agent onboarding** target (§5) with hierarchical `interests` chips + niche-down follow-ups + function-calling profile extraction → `user_interest_profile`. Email-only passwordless sign-in. Profile sheet + Following / "what's new". **DoD:** a user can be interviewed into a profile, sign in by magic link, and talk to any story hands-free.

> **Next:** Create `reference/supabase-schema.md` defining the cited tables (`segments`, `stories`, `digests`, `caption_sentences`, `detail_chunks`, `story_trust`, `story_timeline`, `story_qa`, `story_topics`, `suggested_questions`, `follows`, `saves`, `player_signals`, `interests`, `user_interest_profile`) so the surface-table data columns are concrete — then `/plan-phases` to turn M-Reel / M-Detail+Q&A / M-Voice+Onboarding into phases.
